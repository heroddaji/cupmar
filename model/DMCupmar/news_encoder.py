...
class NewsEncoder(DMBase):
    def __init__(self, config, pretrained_word_embedding=None):
        ...
        self.cate_linear = nn.Linear(config.category_embedding_dim * 2, config.category_embedding_dim)
        self.title_entity_linear = nn.Linear(config.entity_embedding_dim, config.entity_embedding_dim)
        self.abstract_entity_linear = nn.Linear(config.entity_embedding_dim, config.entity_embedding_dim)
        ...
        if config.use_pretrain_word_embedding:
            ...
            self.word_embedding = nn.Embedding(config.num_words, config.word_embedding_dim,
                                               padding_idx=0)
        self.title_abstract_linear = nn.Linear(config.word_embedding_dim, config.category_embedding_dim)
    ...
    def forward(self, news):
        ...
        batch_cate_subcate_tensor = F.relu(
            self.cate_linear(torch.cat((batch_cate_tensor, batch_subcate_tensor), dim=1)))
        ...
        batch_entities_title_tensor = F.relu(self.title_entity_linear(batch_entities_title_tensor))
        ...
        batch_entities_abstract_tensor = F.relu(self.abstract_entity_linear(batch_entities_abstract_tensor))

        # batch_title_abstract_entity_tensor = F.relu(
        #     self.entity_linear(torch.cat((batch_entities_title_tensor, batch_entities_abstract_tensor), dim=1)))
        ...
        # batch_size, category_embedding_dim
        batch_title_abstract_tensor = F.relu(self.title_abstract_linear(batch_title_abstract_tensor))

        # batch_size,  category_embedding_dim
        batch_title_abstract_tensor = self.multihead_self_attention(batch_title_abstract_tensor)

        final_news_tensor = torch.cat((batch_cate_subcate_tensor.unsqueeze(1),
