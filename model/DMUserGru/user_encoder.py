...
class UserEncoder(DMBase):
    ...
    def forward(self, batch_users, batch_clicked_news, empty_clicked_news_batch_size=1):
        ...
        if len(batch_clicked_news) == 0:
            ...
            batch_clicked_news_tensor = torch.stack([self.news_encoder(news) for news in batch_clicked_news]).to(self.device)

        output, hidden = self.gru(batch_clicked_news_tensor, user_embedding_tensor.unsqueeze(0))
