...
class NewsEncoder(torch.nn.Module):
    ...
    def forward(self, news):
        ...
        category_vector = self.category_embedding(news['category'].to(self.device))
        ...
        subcategory_vector = self.category_embedding(
            news['subcategory'].to(self.device))
        # batch_size, num_filters
        ...
        title_vector = F.dropout(self.word_embedding(
            torch.stack(news['title'], dim=1).to(self.device)),
        ...
        abstract_vector = F.dropout(self.word_embedding(
            torch.stack(news['abstract'], dim=1).to(self.device)),
            p=self.config.dropout_probability,
            training=self.training)
