...
from model.DMCategory.news_encoder import NewsEncoder
from model.DMCategory.user_encoder import UserEncoder
from model.general.click_predictor.dot_product import DotProductClickPredictor


class DMCategory(DMBase):
    def __init__(self, config, pretrained_word_embedding=None, writer=None):
        super(DMCategory, self).__init__(config)
        self.news_encoder = NewsEncoder(self.config)
    ...
    def forward(self, batch_impressions):
        ...
        user_vector = self.user_encoder(clicked_news)
    ...
    def get_user_vector(self, clicked_news, empty_clicked_news_batch_size=1):
        """
        Args:
            clicked_news: batch_size, num_clicked_news_a_user, cate_embed_dim
        Returns:
            (shape) batch_size, cate_embed_dim
        """
        # batch_size, cate_embed_dim
        return self.user_encoder(clicked_news, empty_clicked_news_batch_size)
